...
const MinimalistPortfolio = () => {
  ...
  return (
    <div className={`min-h-screen bg-custom-beige text-gray-800 font-sans antialiased ${isLoaded ? 'opacity-100' : 'opacity-0'} transition-opacity duration-500`}>
      {/* Letterhead Header */}
      <header className="py-12 px-8 md:px-16 border-b border-custom-light-orange relative">
        {/* <div className="absolute left-0 right-0 h-1 bg-orange-200 bottom-0"></div> */}
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            ...
            <a href={`mailto:${personal.email}`} className="hover:text-gray-800 transition-colors duration-200 flex items-center justify-start">
                <svg className="w-4 h-4 mr-1" style={{ color: '#dd622b' }} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            ...
            <a href={`https://${personal.github}`} target="_blank" rel="noopener noreferrer" className="hover:text-gray-800 transition-colors duration-200 flex items-center justify-start">
                <svg className="w-4 h-4 mr-1" style={{ color: '#dd622b' }} fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <section>
            ...
            <div className="space-y-16">
              {projects.map(project => (
                <div 
                  key={project.id}
                  className="border-l-2 border-custom-light-orange pl-6 py-2 transform transition-all duration-300 hover:border-custom-orange"
                ...
                >
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {project.tech.map((tech, index) => (
                          <span key={index} className="text-xs px-2 py-1 bg-custom-light-orange rounded">{tech}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <a 
                    ...
                    className={`inline-block mt-2 text-sm text-gray-500 border-b border-custom-orange hover:text-custom-orange hover:border-custom-orange transition-all duration-200 ${activeProject === project.id ? 'opacity-100' : 'opacity-70'}`}
                  >
      ...
      {/* Footer */}
      <footer className="py-8 px-8 md:px-16 border-t border-custom-light-orange mt-16 relative">
        {/* <div className="absolute left-0 right-0 h-1 bg-orange-200 top-0"></div> */}
